            ...
            for entry in full_data:
                ...
                for hand_key in ['0', '1']:
                    if hand_key in entry['landmarks']:
                        ...
                        # 21 keypoints × 3 values (x, y, z) per missing hand
                        flat.extend([0.0] * 63)
...
# ---------------------------------------------------------------------------
# Feature Extraction — flat format only
# ---------------------------------------------------------------------------
...
def run_extractor(dataset, feature_filename, label_filename,
    ...
    Output — flat format only (total_windows, 6336):
        feature_filename  → features
        label_filename    → labels (one per window)

    Checkpointing:
        A partial save is written every checkpoint_every clips so a crash
        does not lose all progress.
    """
    all_features = []   # every window as a row
    all_labels   = []   # label repeated per window
    ...
    for idx in range(total):
        ...
        # Extract visual features for all windows in this clip
        ...
        # Checkpoint every N clips so a crash doesn't lose everything
    ...
    # Save flat format
    final_matrix = np.concatenate(all_features, axis=0)   # (total_windows, 6336)
